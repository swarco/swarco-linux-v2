...
#
#############################################################
BR2_DEBIAN_MIRROR:=ftp.de.debian.org
ZCAT:=zcat
...
$(LIGHTTPD_DIR)/.unpacked: $(DL_DIR)/$(LIGHTTPD_SOURCE) $(LIGHTTPD_PATCH_FILE)
...
$(TARGET_DIR)/$(LIGHTTPD_TARGET_BINARY): $(LIGHTTPD_DIR)/$(LIGHTTPD_BINARY)
	$(INSTALL) $(LIGHTTPD_DIR)/src/lighttpd $(TARGET_DIR)/$(LIGHTTPD_TARGET_BINARY)
	$(INSTALL) -d $(TARGET_DIR)/etc/lighttpd
	$(INSTALL) package/lighttpd/lighttpd.conf $(TARGET_DIR)/etc/lighttpd/lighttpd.conf
	$(INSTALL) -d $(TARGET_DIR)/usr/lib/lighttpd

	cp $(LIGHTTPD_DIR)/src/.libs/mod_indexfile.so $(TARGET_DIR)/usr/lib/lighttpd
	cp $(LIGHTTPD_DIR)/src/.libs/mod_dirlisting.so $(TARGET_DIR)/usr/lib/lighttpd
	cp $(LIGHTTPD_DIR)/src/.libs/mod_staticfile.so $(TARGET_DIR)/usr/lib/lighttpd
	cp $(LIGHTTPD_DIR)/src/.libs/mod_cgi.so $(TARGET_DIR)/usr/lib/lighttpd

	# .. you may install more modules here ...
